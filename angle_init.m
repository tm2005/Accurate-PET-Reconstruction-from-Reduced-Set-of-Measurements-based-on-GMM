function [phi0] = angle_init(phii,rci,s1,s2)
    ai = 2*phii;
    
    M  = (s2^2-s1^2);
    Ni = s1^2+s2^2 - 2*rci.^2;

    %%
    
    As2 = M*sum(sin(ai).*cos(ai));
%     Ac2 = -As2;
    Asc = M*sum(cos(ai).^2) - M*sum(sin(ai).^2);
    As  = sum(Ni.*cos(ai));
    Ac  = -sum(Ni.*sin(ai));
...
%     xr  = double(roots_4(vpa(plnm)));
    ...
    for i = 1:length(phi00s)
        phi00 = phi00s(i);
        s0(i) = sum( (s1.^2*sin(phi00-phii).^2 + s2.^2*cos(phi00-phii).^2 - rci.^2).^2 );
    end
    [~,ind] = min(s0);
    phi0 = phi00s(ind);
